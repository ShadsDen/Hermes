...
1) Parent and VM dependencies and install scripts.
   ...
   Cuda Drivers and tools.APT version locking for Tesla V100
...
6) PostgreSQL Local Server (dedicated VM).

        PGadmin4 (WebUI).

7) Hindsight Local External (dedicated VM).

        Control Plane (WebUI).
  
9) LLM Hosts on Parent Server.
...
10) Agent to agent API scripts.

11) Completed instruction and script to rebuild from scratch.
